...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("bolt-code")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (bolt-code-nightly) when accessing VSCode configuration", async () => {
			...
			;(pkg.Package as any).name = "bolt-code-nightly"
			...
			try {
				...
				expect(mockGetConfiguration).toHaveBeenCalledWith("bolt-code-nightly")
				expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
			} finally {
