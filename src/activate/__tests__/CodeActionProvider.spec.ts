...
vi.mock("../../i18n", () => ({
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"common:codeActions.explain": "Explain with Bolt Code",
			"common:codeActions.fix": "Fix with Bolt Code",
			"common:codeActions.improve": "Improve with Bolt Code",
			"common:codeActions.addToContext": "Add to Bolt Code",
		}
...
describe("CodeActionProvider", () => {
	...
	describe("provideCodeActions", () => {
		it("should provide explain, improve, fix logic, and add to context actions by default", () => {
			...
			expect((actions as any)[0].title).toBe("Add to Bolt Code")
			expect((actions as any)[1].title).toBe("Explain with Bolt Code")
			expect((actions as any)[2].title).toBe("Improve with Bolt Code")
		})
		...
		it("should provide fix action instead of fix logic when diagnostics exist", () => {
			...
			expect((actions as any).some((a: any) => a.title === "Fix with Bolt Code")).toBe(true)
			expect((actions as any).some((a: any) => a.title === "Add to Bolt Code")).toBe(true)
		})
		...
		it("should return empty array when enableCodeActions is disabled", () => {
			...
			expect(actions).toEqual([])
			expect(vscode.workspace.getConfiguration).toHaveBeenCalledWith("bolt-code")
