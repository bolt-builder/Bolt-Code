...
describe("registerCommands handlers", () => {
	...
	it("settingsButtonClicked posts both settingsButtonClicked and didBecomeVisible actions", () => {
		handlers["bolt-code.settingsButtonClicked"]()
	...
	it("historyButtonClicked posts historyButtonClicked action", () => {
		handlers["bolt-code.historyButtonClicked"]()
	...
	it("marketplaceButtonClicked posts marketplaceButtonClicked action", () => {
		handlers["bolt-code.marketplaceButtonClicked"]()
	...
	it("acceptInput posts acceptInput message", () => {
		handlers["bolt-code.acceptInput"]()
	...
	it("toggleAutoApprove awaits postMessage with toggleAutoApprove action", async () => {
		...
		const handlerPromise = handlers["bolt-code.toggleAutoApprove"]() as Promise<unknown>
		let settled = false
	...
	it("focusInput awaits postMessage on the registered provider when a sidebar panel is active", async () => {
		...
		const handlerPromise = handlers["bolt-code.focusInput"]() as Promise<unknown>
		let settled = false
		void handlerPromise.then(() => {
			settled = true
		})
	...
	it("focusInput does not post when no sidebar panel is active", async () => {
		await handlers["bolt-code.focusInput"]()
	...
	it.each([
		{ command: "bolt-code.settingsButtonClicked", prefix: "settingsButtonClicked", expectedCalls: 2 },
		{ command: "bolt-code.historyButtonClicked", prefix: "historyButtonClicked", expectedCalls: 1 },
		{ command: "bolt-code.marketplaceButtonClicked", prefix: "marketplaceButtonClicked", expectedCalls: 1 },
		{ command: "bolt-code.acceptInput", prefix: "acceptInput", expectedCalls: 1 },
	])(
	...
	it("toggleAutoApprove logs to outputChannel when postMessageToWebview rejects", async () => {
		...
		await handlers["bolt-code.toggleAutoApprove"]()
	...
	it("plusButtonClicked calls evictCurrentTask on the visible provider", async () => {
		...
		await handlers["bolt-code.plusButtonClicked"]()
