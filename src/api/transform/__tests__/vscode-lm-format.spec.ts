...
describe("convertToVsCodeLmMessages", () => {
	...
	it("should log Bolt Code branded warning when asObjectSafe fails to parse invalid JSON string", () => {
		const consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => {})
		...
		expect(consoleWarnSpy).toHaveBeenCalledWith(
			"Bolt Code <Language Model API>: Failed to parse object:",
			expect.any(Error),
		)
...
describe("extractTextCountFromMessage", () => {
	...
	it("should log Bolt Code branded warning when tool call input stringify fails", () => {
		...
		expect(consoleErrorSpy).toHaveBeenCalledWith(
			"Bolt Code <Language Model API>: Failed to stringify tool call input:",
			expect.any(Error),
		)
