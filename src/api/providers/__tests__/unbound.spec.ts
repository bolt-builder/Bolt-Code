...
describe("UnboundHandler", () => {
	...
	it("identifies itself as Bolt Code in the Unbound request headers", () => {
		new UnboundHandler({
		...
		expect(OpenAI).toHaveBeenCalledWith(
			expect.objectContaining({
				defaultHeaders: expect.objectContaining({
					"X-Unbound-Metadata": JSON.stringify({ labels: [{ key: "app", value: "bolt-code" }] }),
				}),
			}),
		)
	})
	...
	it("identifies itself as Bolt Code in per-request Unbound metadata", async () => {
		const mockCreate = (OpenAI as unknown as any)().chat.completions.create
		mockCreate.mockResolvedValue(
			asyncStreamFrom([
				{
