...
export const mentionRegex =
	/(?:^|(?<=\s))(?<!\\)@((?:\/|\w+:\/\/)(?:[^\s\\]|\\ )+?|(?:search|codebase|skill|task|diff|symbol):(?:[^\s\\]|\\ )+?|[a-f0-9]{7,40}\b|problems\b|git-changes\b|terminal\b|selection\b|tabs\b|tab\b|clipboard\b|recent\b|tree\b)(?=[.,;:!?]?(?=[\s\r\n]|$))/
