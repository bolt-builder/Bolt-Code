...
export async function getRipgrepDiagnostic(vscodeAppRoot: string): Promise<string> {
	...
	const lines: string[] = [
		`Bolt Code Ripgrep Diagnostic (${new Date().toISOString()})`,
...
/**
 * Registers the `bolt-code.showRipgrepDiagnostic` command. Thin wrapper —
...
export function registerRipgrepDiagnosticCommand(): vscode.Disposable {
	const channel = vscode.window.createOutputChannel("Bolt Code Ripgrep Diagnostic")
	const command = vscode.commands.registerCommand(getCommand("showRipgrepDiagnostic"), async () => {
		...
		await vscode.window.showInformationMessage("Bolt Code: ripgrep diagnostic copied to clipboard.")
	})
