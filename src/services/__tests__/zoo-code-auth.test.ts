...
describe("zoo-code-auth", () => {
	...
	describe("getCachedZooCodeToken", () => {
		...
		it("preloads the cached token during initialization", async () => {
			await mockSecrets.store("bolt-code-session-token", "zoo_ext_cached_token")
	...
	describe("initZooCodeAuth", () => {
		it("clears stored user info and token when the cached token is invalid", async () => {
			await mockSecrets.store("bolt-code-session-token", "zoo_ext_stale_token")
			await mockSecrets.store("bolt-code-user-name", "Jane Doe")
			await mockSecrets.store("bolt-code-user-email", "jane@example.com")
			await mockSecrets.store("bolt-code-user-image", "https://example.com/avatar.png")
			mockFetch.mockResolvedValueOnce({
				ok: true,
		...
		it("preserves token and user info when the backend is temporarily unreachable", async () => {
			await mockSecrets.store("bolt-code-session-token", "zoo_ext_valid_token")
			await mockSecrets.store("bolt-code-user-name", "Jane Doe")
			await mockSecrets.store("bolt-code-user-email", "jane@example.com")
	...
	describe("handleAuthCallback", () => {
		it("does not persist a token when backend verification fails", async () => {
			...
			expect(getCachedZooCodeToken()).toBe("")
			expect(mockSecrets.store).not.toHaveBeenCalledWith("bolt-code-session-token", "zoo_ext_fake_token")
		})
		...
		it("persists a token only after backend verification succeeds", async () => {
			...
			expect(mockSecrets.store).toHaveBeenCalledWith("bolt-code-session-token", "zoo_ext_real_token")
		})
	})
