...
/**
 * Check if we should auto-open the Bolt Code sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening Bolt Code sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the Bolt Code sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("bolt-code.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize Bolt Code auth service for extension session token management.
