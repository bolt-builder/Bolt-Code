...
export class Terminal extends BaseTerminal {
	...
	constructor(id: number, terminal: vscode.Terminal | undefined, cwd: string) {
		...
		if (terminal) {
			...
			const options: vscode.TerminalOptions = { cwd, name: "Bolt Code", iconPath, env }
			...
			if (profileShell?.shellPath) {
				...
				// Bolt Code's shell-integration controls authoritative.
				if (profileShell.env) {
	 ...
	 * `.vscode/settings.json` to select an executable for Bolt Code to launch.
	 */
