...
The Bolt Code extension exposes an API that can be used by other extensions.
...
> extension rebrand to Bolt Code.
...
const extension = vscode.extensions.getExtension<RooCodeAPI>("bolt-builder.bolt-code")
...
await api.startNewTask("Hello, Bolt Code API! Let's make a new project...")
...
**NOTE:** To ensure that the `bolt-builder.bolt-code` extension is activated before your extension, add it to the `extensionDependencies` in your `package.json`:

```json
"extensionDependencies": ["bolt-builder.bolt-code"]
```
